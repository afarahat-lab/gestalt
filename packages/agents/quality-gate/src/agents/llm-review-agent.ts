...
function buildReviewPrompt(
  ...
  /**
   * TR_036 — Auto-generated project-structure brief
   * (ARCHITECTURE.md + src/ directory tree) assembled by the
   * gate-orchestrator. Injected BEFORE the persona + rules so the
   * agent can map abstract layer-role rules to the actual paths.
   * Empty/undefined when the project has no ARCHITECTURE.md and no
   * src/ tree.
   */
  ...
  // TR_036 — inject the auto-generated project-structure brief
  // (ARCHITECTURE.md + src/ directory tree) at the top so the
  // agent can map abstract layer-role rules to actual paths.
  const structureSection = projectStructureBrief && projectStructureBrief.length > 0
    ? `${projectStructureBrief}\n`
    : '';

  return `${persona}
${structureSection}You are reviewing code generated by upstream agents. Your job is to
