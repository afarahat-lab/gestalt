...
Return ONLY a JSON object — no preamble, no markdown fences:
{
  ...
  "fixIntent": "if action is fix-intent: describe the CONTEXT and FAILURE that needs resolving. Include: (1) the exact CI error text, (2) which file contains the problem, (3) what the code was trying to do when it failed. Do NOT write prescriptive instructions — provide context, let Aider analyse and decide the fix. Frame the problem as a BROKEN STATE, not a MISSING STATE — Aider inverts negation, so 'X does not exist' tends to make Aider CREATE X. WRONG framing: 'ILeaveRepository does not exist in the module'. CORRECT framing: 'The service imports ILeaveRepository but the repository file exports LeaveRepository (no I prefix). The import path is wrong.' This prevents Aider creating new files when the fix is an import correction or type rename. ALWAYS end the fixIntent with this exact sentence: 'Preserve all existing exports, types, interfaces, and imports. Only add or modify what is needed to resolve the CI failure shown above.'",
